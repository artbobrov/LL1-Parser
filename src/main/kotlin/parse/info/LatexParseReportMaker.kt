...
\textsl{Input string:} ${matchInfo.originalText}
...
${treeFormatter.format(tree)}
    ...
    private fun formatError(error: ParseError?): String {
        ...
        return """
Error occurred! ${parseError.message ?: ""}
        """.trimIndent()
    }
...
""".trimIndent()
    ...
    private fun formatNullableFirstFollowTable(
        ...
        val nullableColumn = "Derives ${formatted(Symbol.EMPTY)}"
        val firstColumn = "FIRST table"
        val followColumn = "FOLLOW table"
        ...
        return formatTable("Additional tables:", rows, columns, tableContent)
    }
    ...
    private fun formatLL1ParseTable(parseTable: LL1Table): String {
        ...
        return formatTable("LL(1) table", rows, columns, tableContent)
    }
